...
public class SteamVR_TrackedController : MonoBehaviour
{
    ...
    protected virtual void Update()
    {
        ...
        {
            var e = new ClickedEventArgs()
            {
                controllerIndex = controllerIndex,
                padX = controllerState.rAxis0.x,
                padY = controllerState.rAxis0.y,
                flags = (uint)controllerState.ulButtonPressed,
            };
            ulong trigger = controllerState.ulButtonPressed & (1UL << ((int)EVRButtonId.k_EButton_SteamVR_Trigger));
            ulong grip = controllerState.ulButtonPressed & (1UL << ((int)EVRButtonId.k_EButton_Grip));
            ulong pad = controllerState.ulButtonPressed & (1UL << ((int)EVRButtonId.k_EButton_SteamVR_Touchpad));
            ulong menu = controllerState.ulButtonPressed & (1UL << ((int)EVRButtonId.k_EButton_ApplicationMenu));
            var touchPad = controllerState.ulButtonTouched & (1UL << ((int)EVRButtonId.k_EButton_SteamVR_Touchpad));
            ...
            if (touchPad > 0L && !padTouched)
            {
                padTouched = true;
                e.flags = (uint)controllerState.ulButtonTouched;
                OnPadTouched(e);
            }
            else if (touchPad == 0L && padTouched)
            {
                padTouched = false;
                e.flags = (uint)controllerState.ulButtonTouched;
                OnPadUntouched(e);
