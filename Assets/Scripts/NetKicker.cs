...
namespace UnityEngine.Networking
{
	...
	public class NetKicker : MonoBehaviour
	{
		...
		void OnGUI()
		{
			...
			if (!manager.IsClientConnected() && !NetworkServer.active && manager.matchMaker == null)
			{
				if (noConnection)
				{
					if (UnityEngine.Application.platform != RuntimePlatform.WebGLPlayer)
					{
						if (GUI.Button(new Rect(xpos, ypos, 200, 200), "LAN Host(H)"))
						{
							manager.StartHost();
						}
						//ypos += spacing;
					...
					if (GUI.Button(new Rect(xpos +200, ypos, 200, 200), "LAN Client(C)"))
					{
						manager.StartClient();
					}

					manager.networkAddress = GUI.TextField(new Rect(xpos + 100, ypos+400, 95, 20), manager.networkAddress);
					ypos += spacing;
